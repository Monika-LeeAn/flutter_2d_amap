...
- (void)initAnnotations:(NSMutableArray *)annotationsFormFlutter level:(NSString *)customMapLevel
{
    ...
    for (int i = 0; i < annotationsFormFlutter.count; ++i)
    {
        ...
        if ([customMapLevel  isEqual: @"1"]){
                ...
                double lat = [annotationsFormFlutter[i][@"cityLatitude"] doubleValue];
                double lng = [annotationsFormFlutter[i][@"cityLongitude"] doubleValue];
                CLLocationCoordinate2D coordinate = CLLocationCoordinate2DMake(lat, lng);
                a1.coordinate = coordinate;
                a1.title      = [NSString stringWithFormat:@"city:%d", i];
        } else if ([customMapLevel isEqual: @"3"]){
                double lat = [annotationsFormFlutter[i][@"operateLatitude"] doubleValue];
                double lng = [annotationsFormFlutter[i][@"operateLongitude"] doubleValue];
                CLLocationCoordinate2D coordinate = CLLocationCoordinate2DMake(lat, lng);
                a1.coordinate = coordinate;
...
- (void)mapView:(MAMapView *)mapView didSelectAnnotationView:(MAAnnotationView *)view
{
    ...
    } else if ([view isKindOfClass:[ProvinceAnnotationView class]]) {
        ProvinceAnnotationView *pView = (ProvinceAnnotationView*)view;
        NSDictionary* arguments = @{
...
//        }
        MerchantAnnotationView *pView = (MerchantAnnotationView*)view;
        NSDictionary* arguments = @{
                                    @"model" : pView.modelDictionry,
                                    @"level": _currentLevel,
                                    };
