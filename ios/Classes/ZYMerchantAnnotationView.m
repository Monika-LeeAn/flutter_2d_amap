...
static CGFloat const kJPSThumbnailAnnotationViewStandardWidth     = 45.0f;
...
static CGFloat const kJPSThumbnailAnnotationViewVerticalOffset    = 15.0f;
...
//    self.subtitleLabel.text =@"klajdklasjlk";
    ...
    self.mainLabel.text = modelDictionry[@"name"];
...
//    self.mainLabel.text  = @"这是用于测阿萨大";
       
    if (self.mainLabel.text.length > 0){
        self.mainLabel.numberOfLines = 0;
           
           NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:20],};
           CGSize textSize = [self.mainLabel.text boundingRectWithSize:CGSizeMake(100, 100) options:NSStringDrawingTruncatesLastVisibleLine attributes:attributes context:nil].size;;

        [self.mainLabel setFrame:CGRectMake(10, 10, textSize.width, textSize.height)];
     [self setFrame:CGRectMake(0, 0, textSize.width + 20, textSize.height + 25)];
...
- (id)initWithAnnotation:(id<MAAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.centerOffset = CGPointMake(-1.3, -kJPSThumbnailAnnotationViewVerticalOffset);
        [self setupView];
