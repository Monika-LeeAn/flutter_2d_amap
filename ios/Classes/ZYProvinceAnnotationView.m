...
- (void)didTapDisclosureButton {
    ...
    if (self.state == ZYProvinceAnnotationViewStateExpanded){
...
//    if(self.state == ZYProvinceAnnotationViewStateExpanded){
//        [self shrink];
//    } else if (self.state == ZYProvinceAnnotationViewStateCollapsed){
//        [self expand];
//    }
////    [self touch]
//}
...
- (void) setUpDisclosureImage{
    ...
    UIImage *disclosureIndicatorImage = [ZYProvinceAnnotationView disclosureButtonImage];
...
- (void)expand {
    if (self.state != ZYProvinceAnnotationViewStateCollapsed) return;

    self.state = ZYProvinceAnnotationViewStateAnimating;
//    self.mainLabel.hidden = YES;

    [self animateBubbleWithDirection:ZYProvinceViewAnimationDirectionGrow];
    ...
    [UIView animateWithDuration:kJPSThumbnailAnnotationViewAnimationDuration/2.0f delay:kJPSThumbnailAnnotationViewAnimationDuration options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        self.state = ZYProvinceAnnotationViewStateExpanded;
    }];
}

- (void)shrink {
    if (self.state != ZYProvinceAnnotationViewStateExpanded) return;
    ...
    [UIView animateWithDuration:kJPSThumbnailAnnotationViewAnimationDuration/2.0f
                     ...
                     completion:^(BOOL finished) {
                         [self animateBubbleWithDirection:ZYProvinceViewAnimationDirectionShrink];
                         self.centerOffset = CGPointMake(0.0f, -kJPSThumbnailAnnotationViewVerticalOffset);
                     }];
}

-(void)animateBubbleWithDirection:(ZYProvinceAnnotationViewAnimationDirection)animationDirection {
    BOOL growing = (animationDirection == ZYProvinceViewAnimationDirectionGrow);
    // Image
    [UIView animateWithDuration:kJPSThumbnailAnnotationViewAnimationDuration animations:^{
        ...
        if (animationDirection == ZYProvinceViewAnimationDirectionShrink) {
            self.state = ZYProvinceAnnotationViewStateCollapsed;
        }
    }];
